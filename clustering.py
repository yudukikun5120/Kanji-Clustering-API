...
def extract_feature(set: str) -> np.ndarray:
    return np.array(
        [ndarray_of(char) for char in progressbar.progressbar(kanji_group(set))]
    ).reshape(len(kanji_group(set)), -1)


def create_fitted_estimator(set: str, n_clusters: int = 100) -> (KMeans, pd.DataFrame):
    ...
    df = pd.concat([pd.Series(kanji_group(set), name="character"), labels], axis=1)
